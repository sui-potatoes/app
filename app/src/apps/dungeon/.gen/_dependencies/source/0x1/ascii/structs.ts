...
export function isChar(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V6}::ascii::Char`; }

export interface CharFields { byte: ToField<"u8"> }

export type CharReified = Reified< Char, CharFields >;

export class Char implements StructClass { static readonly $typeName = `${PKG_V6}::ascii::Char`; static readonly $numTypeParams = 0;
 ...
 private constructor(typeArgs: [], fields: CharFields, ) { this.$fullTypeName = composeSuiType( Char.$typeName, ...typeArgs ) as `${typeof PKG_V6}::ascii::Char`; this.$typeArgs = typeArgs;

 this.byte = fields.byte; }

 static reified( ): CharReified { return { typeName: Char.$typeName, fullTypeName: composeSuiType( Char.$typeName, ...[] ) as `${typeof PKG_V6}::ascii::Char`, typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Char.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Char.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Char.fromBcs( data, ), bcs: Char.bcs, fromJSONField: (field: any) => Char.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Char.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Char.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => Char.fetch( client, id, ), new: ( fields: CharFields, ) => { return new Char( [], fields ) }, kind: "StructClassReified", } }
...
export function isString(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V6}::ascii::String`; }

export interface StringFields { bytes: ToField<Vector<"u8">> }

export type StringReified = Reified< String, StringFields >;

export class String implements StructClass { static readonly $typeName = `${PKG_V6}::ascii::String`; static readonly $numTypeParams = 0;
