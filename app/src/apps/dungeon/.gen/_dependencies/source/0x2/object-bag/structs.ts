...
export function isObjectBag(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::object_bag::ObjectBag`; }
...
export class ObjectBag implements StructClass { static readonly $typeName = `${PKG_V19}::object_bag::ObjectBag`; static readonly $numTypeParams = 0;
 ...
 private constructor(typeArgs: [], fields: ObjectBagFields, ) { this.$fullTypeName = composeSuiType( ObjectBag.$typeName, ...typeArgs ) as `${typeof PKG_V19}::object_bag::ObjectBag`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.size = fields.size; }

 static reified( ): ObjectBagReified { return { typeName: ObjectBag.$typeName, fullTypeName: composeSuiType( ObjectBag.$typeName, ...[] ) as `${typeof PKG_V19}::object_bag::ObjectBag`, typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ObjectBag.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ObjectBag.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ObjectBag.fromBcs( data, ), bcs: ObjectBag.bcs, fromJSONField: (field: any) => ObjectBag.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ObjectBag.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => ObjectBag.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => ObjectBag.fetch( client, id, ), new: ( fields: ObjectBagFields, ) => { return new ObjectBag( [], fields ) }, kind: "StructClassReified", } }
