...
export function isBag(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::bag::Bag`; }

export interface BagFields { id: ToField<UID>; size: ToField<"u64"> }

export type BagReified = Reified< Bag, BagFields >;

export class Bag implements StructClass { static readonly $typeName = `${PKG_V19}::bag::Bag`; static readonly $numTypeParams = 0;
 ...
 private constructor(typeArgs: [], fields: BagFields, ) { this.$fullTypeName = composeSuiType( Bag.$typeName, ...typeArgs ) as `${typeof PKG_V19}::bag::Bag`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.size = fields.size; }

 static reified( ): BagReified { return { typeName: Bag.$typeName, fullTypeName: composeSuiType( Bag.$typeName, ...[] ) as `${typeof PKG_V19}::bag::Bag`, typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Bag.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Bag.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Bag.fromBcs( data, ), bcs: Bag.bcs, fromJSONField: (field: any) => Bag.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Bag.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Bag.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => Bag.fetch( client, id, ), new: ( fields: BagFields, ) => { return new Bag( [], fields ) }, kind: "StructClassReified", } }
