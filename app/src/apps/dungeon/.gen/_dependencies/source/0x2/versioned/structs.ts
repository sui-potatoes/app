...
export function isVersionChangeCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::versioned::VersionChangeCap`; }
...
export class VersionChangeCap implements StructClass { static readonly $typeName = `${PKG_V19}::versioned::VersionChangeCap`; static readonly $numTypeParams = 0;
 ...
 private constructor(typeArgs: [], fields: VersionChangeCapFields, ) { this.$fullTypeName = composeSuiType( VersionChangeCap.$typeName, ...typeArgs ) as `${typeof PKG_V19}::versioned::VersionChangeCap`; this.$typeArgs = typeArgs;
...
export function isVersioned(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::versioned::Versioned`; }
...
export class Versioned implements StructClass { static readonly $typeName = `${PKG_V19}::versioned::Versioned`; static readonly $numTypeParams = 0;
 ...
 private constructor(typeArgs: [], fields: VersionedFields, ) { this.$fullTypeName = composeSuiType( Versioned.$typeName, ...typeArgs ) as `${typeof PKG_V19}::versioned::Versioned`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.version = fields.version; }

 static reified( ): VersionedReified { return { typeName: Versioned.$typeName, fullTypeName: composeSuiType( Versioned.$typeName, ...[] ) as `${typeof PKG_V19}::versioned::Versioned`, typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Versioned.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Versioned.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Versioned.fromBcs( data, ), bcs: Versioned.bcs, fromJSONField: (field: any) => Versioned.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Versioned.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Versioned.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => Versioned.fetch( client, id, ), new: ( fields: VersionedFields, ) => { return new Versioned( [], fields ) }, kind: "StructClassReified", } }
