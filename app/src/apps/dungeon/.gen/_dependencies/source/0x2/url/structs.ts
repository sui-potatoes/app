...
export function isUrl(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::url::Url`; }

export interface UrlFields { url: ToField<String> }

export type UrlReified = Reified< Url, UrlFields >;

export class Url implements StructClass { static readonly $typeName = `${PKG_V19}::url::Url`; static readonly $numTypeParams = 0;
