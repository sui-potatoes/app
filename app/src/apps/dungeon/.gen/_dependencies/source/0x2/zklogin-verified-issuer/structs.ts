...
export function isVerifiedIssuer(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::zklogin_verified_issuer::VerifiedIssuer`; }
...
export class VerifiedIssuer implements StructClass { static readonly $typeName = `${PKG_V19}::zklogin_verified_issuer::VerifiedIssuer`; static readonly $numTypeParams = 0;
 ...
 private constructor(typeArgs: [], fields: VerifiedIssuerFields, ) { this.$fullTypeName = composeSuiType( VerifiedIssuer.$typeName, ...typeArgs ) as `${typeof PKG_V19}::zklogin_verified_issuer::VerifiedIssuer`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.owner = fields.owner;; this.issuer = fields.issuer; }

 static reified( ): VerifiedIssuerReified { return { typeName: VerifiedIssuer.$typeName, fullTypeName: composeSuiType( VerifiedIssuer.$typeName, ...[] ) as `${typeof PKG_V19}::zklogin_verified_issuer::VerifiedIssuer`, typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => VerifiedIssuer.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VerifiedIssuer.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => VerifiedIssuer.fromBcs( data, ), bcs: VerifiedIssuer.bcs, fromJSONField: (field: any) => VerifiedIssuer.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => VerifiedIssuer.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => VerifiedIssuer.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => VerifiedIssuer.fetch( client, id, ), new: ( fields: VerifiedIssuerFields, ) => { return new VerifiedIssuer( [], fields ) }, kind: "StructClassReified", } }
