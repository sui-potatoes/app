...
export function isElement(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V19}::group_ops::Element` + '<'); }
...
export class Element<T extends PhantomTypeArgument> implements StructClass { static readonly $typeName = `${PKG_V19}::group_ops::Element`; static readonly $numTypeParams = 1;
