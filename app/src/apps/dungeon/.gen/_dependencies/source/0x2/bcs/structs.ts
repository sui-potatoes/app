...
export function isBCS(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::bcs::BCS`; }

export interface BCSFields { bytes: ToField<Vector<"u8">> }

export type BCSReified = Reified< BCS, BCSFields >;

export class BCS implements StructClass { static readonly $typeName = `${PKG_V19}::bcs::BCS`; static readonly $numTypeParams = 0;
