...
export function isBalance(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V19}::balance::Balance` + '<'); }
...
export class Balance<T extends PhantomTypeArgument> implements StructClass { static readonly $typeName = `${PKG_V19}::balance::Balance`; static readonly $numTypeParams = 1;
...
export function isSupply(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V19}::balance::Supply` + '<'); }
...
export class Supply<T extends PhantomTypeArgument> implements StructClass { static readonly $typeName = `${PKG_V19}::balance::Supply`; static readonly $numTypeParams = 1;
