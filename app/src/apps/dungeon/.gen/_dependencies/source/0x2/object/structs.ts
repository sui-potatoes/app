...
export function isID(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::object::ID`; }

export interface IDFields { bytes: ToField<"address"> }

export type IDReified = Reified< ID, IDFields >;

export class ID implements StructClass { static readonly $typeName = `${PKG_V19}::object::ID`; static readonly $numTypeParams = 0;
 ...
 private constructor(typeArgs: [], fields: IDFields, ) { this.$fullTypeName = composeSuiType( ID.$typeName, ...typeArgs ) as `${typeof PKG_V19}::object::ID`; this.$typeArgs = typeArgs;

 this.bytes = fields.bytes; }

 static reified( ): IDReified { return { typeName: ID.$typeName, fullTypeName: composeSuiType( ID.$typeName, ...[] ) as `${typeof PKG_V19}::object::ID`, typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ID.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ID.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ID.fromBcs( data, ), bcs: ID.bcs, fromJSONField: (field: any) => ID.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ID.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => ID.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => ID.fetch( client, id, ), new: ( fields: IDFields, ) => { return new ID( [], fields ) }, kind: "StructClassReified", } }
...
export function isUID(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V19}::object::UID`; }

export interface UIDFields { id: ToField<ID> }

export type UIDReified = Reified< UID, UIDFields >;

export class UID implements StructClass { static readonly $typeName = `${PKG_V19}::object::UID`; static readonly $numTypeParams = 0;
