...
export function isReceiving(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V19}::transfer::Receiving` + '<'); }
...
export class Receiving<T extends PhantomTypeArgument> implements StructClass { static readonly $typeName = `${PKG_V19}::transfer::Receiving`; static readonly $numTypeParams = 1;
