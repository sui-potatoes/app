...
/// Formula is a simple formula builder that can be used to create complex
/// mathematical expressions. It is designed to be used in a type-safe manner
/// and can be used to calculate values of different types.
module spots::formula {
    use sui::math;
    ...
    public fun calculate_u64(self: Formula<u64>, value: u64): u64 {
        let Formula { mut expressions, scaling: _ } = self;
        let scaling = scaling.destroy_with_default(2 << 32) as u128
    ...
    public fun calculate_u128(self: Formula<u128>, value: u128): Result<u256, u128> {
        let Formula { mut expressions, scaling } = self;
        let scaling = scaling.destroy_with_default(2 << 64) as u256;
        let mut is_scaled = false;
        ...
        assert!(result < 340_282_366_920_938_463_463_374_607_431_768_211_455u256 * scaling, EOverflow);
    ...
    public fun unwrap(self: Result<u256, u128>): u128 {
        self.value
    }
    ...
    // copying implementations from the math module / community library
    ...
    fun min_u256(x: u256, y: u256): u256 {
        if (x < y) {
            x
        } else {
            y
